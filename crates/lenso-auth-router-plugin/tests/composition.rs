...
use lenso_auth_router_plugin::{AuthRouterConfig, PACKAGE_ID};
...
impl NativePluginFactory for RejectingFactory {
    fn package_id(&self) -> &'static str {
        self.package_id
    }
...
fn caller() -> PluginInstancePlan {
    PluginInstancePlan::new("caller", CALLER).with_requirement(CapabilityRequirementPlan::one(
        CAPABILITY_ID,
        DESCRIPTOR_VERSION,
    ))
}

fn provider(instance: &str, package: &str) -> PluginInstancePlan {
    PluginInstancePlan::new(instance, package).with_capability(CapabilityEndpointPlan::new(
        CAPABILITY_ID,
        DESCRIPTOR_VERSION,
        [AUTHENTICATE_OPERATION],
    ))
...
fn router_plan() -> ResolvedAppPlan {
    let router = PluginInstancePlan::new("router", PACKAGE_ID)
        .with_configuration(
...
fn full_registry() -> NativePluginRegistry {
    NativePluginRegistry::new()
