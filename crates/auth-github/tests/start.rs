...
#[tokio::test]
async fn start_creates_oauth_flow_and_redirects_to_github() {
    ...
    let response = test_app(db.pool.clone())
        .oneshot(get("/v1/auth/github/start?return_to=/console"))
        .await
    ...
    assert!(location.starts_with("https://github.example.test/login/oauth/authorize?"));
...
fn test_config() -> AppConfig {
    ...
    values.insert(
        ...
        json!("https://github.example.test/login/oauth/authorize"),
    );
    ...
    AppConfig {
        ...
        service: ServiceConfig {
            ...
            name: "auth-github-start-test".to_owned(),
        },
