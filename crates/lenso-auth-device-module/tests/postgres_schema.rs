...
use lenso_auth_device_module::DeviceAuthOperator;
...
async fn external_sql_schema_setup_and_upgrade() {
    ...
    let schema = unique_schema("device");
    let pool = PgPool::connect(&url).await.unwrap();
    DeviceAuthOperator::setup(&url, &schema).await.unwrap();
    DeviceAuthOperator::upgrade(&url, &schema).await.unwrap();
    assert!(table_exists(&pool, &schema, "auth_devices").await);
