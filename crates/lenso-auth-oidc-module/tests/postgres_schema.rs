...
use lenso_auth_oidc_module::OidcOperator;
...
async fn external_sql_schema_setup_and_upgrade() {
    ...
    let schema = unique_schema("oidc");
    let pool = PgPool::connect(&url).await.unwrap();
    OidcOperator::setup(&url, &schema).await.unwrap();
    OidcOperator::upgrade(&url, &schema).await.unwrap();
    assert!(table_exists(&pool, &schema, "oidc_authorization_codes").await);
