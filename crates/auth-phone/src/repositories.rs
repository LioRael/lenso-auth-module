...
use auth_password::password::validate_password;
...
pub struct SetPhonePasswordOptions<'a> {
    ...
    pub password: &'a str,
    pub now: DateTime<Utc>,
    pub config: &'a AuthPasswordConfig,
}
...
impl PhoneAuthRepository {
    ...
    pub async fn set_password(&self, input: SetPhonePasswordOptions<'_>) -> AppResult<bool> {
        ...
        PasswordAuthRepository::new(self.pool.clone())
            .set_identity_password(&identity_id, password, now, config)
            .await?;
    ...
    pub async fn login_password_with_options(
        ...
        let phone_e164 = normalize_phone_e164(phone)?;
        let password_repo = PasswordAuthRepository::new(self.pool.clone());
        password_repo
            .ensure_login_not_locked_for_provider(PHONE_PROVIDER, &phone_e164, now)
            .await?;
        if let Err(error) = validate_public_phone_password(password) {
            password_repo
                .record_failed_login_for_provider(PHONE_PROVIDER, &phone_e164, now, &client)
                .await?;
            return Err(error);
        }
        ...
        if !password_repo
            .verify_identity_password(&identity.id, password)
            .await?
        {
            password_repo
                .record_failed_login_for_provider(PHONE_PROVIDER, &phone_e164, now, &client)
        ...
        tx.commit().await.map_err(map_sql_error)?;
        password_repo
            .clear_login_failures_for_provider(PHONE_PROVIDER, &phone_e164)
            .await?;
...
fn validate_public_phone_password(password: &str) -> AppResult<()> {
    validate_password(password).map_err(|_| invalid_phone_password_credentials())
}
...
mod tests {
    ...
    fn public_phone_password_validation_returns_generic_unauthorized() {
        let error = validate_public_phone_password("short").unwrap_err();
