...
impl NativeModuleFactory for StaticSecretsFactory {
    fn package_id(&self) -> &'static str {
        SECRETS_PACKAGE_ID
    }
...
async fn composed_auth_issues_publicly_verifiable_assertions_and_observes_revocation() {
    ...
    local
        .run_until(async {
            ...
            .await
            .expect("configured Auth Module should prepare");
...
async fn preparation_requires_explicit_setup_and_never_creates_schema() {
    ...
    let exists: bool =
        sqlx::query_scalar("SELECT EXISTS (SELECT 1 FROM pg_namespace WHERE nspname = $1)")
            ...
            .await
            .unwrap();
    assert!(!exists, "Module preparation must not run setup");
}
...
fn plan(schema: &str, public_key_secret: &str) -> ResolvedAppPlan {
    ...
    let auth = ModuleInstancePlan::new("auth", PACKAGE_ID)
