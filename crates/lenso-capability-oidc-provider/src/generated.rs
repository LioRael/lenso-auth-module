...
pub fn encode_jwks_request(value: &JwksRequest) -> Result<String, serde_json::Error> { encode_portable_json(value) }
pub fn decode_jwks_request(wire: &str) -> Result<JwksRequest, serde_json::Error> { decode_portable_json(wire) }
...
pub fn encode_metadata_request(value: &MetadataRequest) -> Result<String, serde_json::Error> { encode_portable_json(value) }
pub fn decode_metadata_request(wire: &str) -> Result<MetadataRequest, serde_json::Error> { decode_portable_json(wire) }
...
impl<P: OidcProviderProvider> NativeRequestEndpoint for OidcProviderEndpoint<P> {
    ...
    fn invoke(&self, operation: &str, request: Box<dyn std::any::Any>, context: InvocationContext) -> LocalBoxFuture<'static, Result<Result<Box<dyn std::any::Any>, Box<dyn std::any::Any>>, RuntimeFailure>> {
        match operation {
            ...
            JWKS_OPERATION => {
                let Ok(request) = request.downcast::<JwksRequest>() else {
                    return Box::pin(futures::future::ready(Err(RuntimeFailure::ProtocolViolation { capability: CAPABILITY_ID })));
                };
            ...
            METADATA_OPERATION => {
                let Ok(request) = request.downcast::<MetadataRequest>() else {
                    return Box::pin(futures::future::ready(Err(RuntimeFailure::ProtocolViolation { capability: CAPABILITY_ID })));
                };
...
impl OidcProviderClient {
    ...
    pub async fn jwks(&self, request: JwksRequest) -> Result<JwksResponse, OidcProviderJwksInvocationError> {
    ...
    pub async fn jwks_with_context(&self, context: InvocationContext, request: JwksRequest) -> Result<JwksResponse, OidcProviderJwksInvocationError> {
        self.jwks.invoke_with_context(JWKS_OPERATION, context, request).await
            .map_err(OidcProviderJwksInvocationError::Runtime)?
            .map_err(OidcProviderJwksInvocationError::Domain)
    }

    pub async fn metadata(&self, request: MetadataRequest) -> Result<MetadataResponse, OidcProviderMetadataInvocationError> {
    ...
    pub async fn metadata_with_context(&self, context: InvocationContext, request: MetadataRequest) -> Result<MetadataResponse, OidcProviderMetadataInvocationError> {
