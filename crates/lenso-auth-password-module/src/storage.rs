...
pub(crate) async fn insert_credential(
    ...
    subject: &str,
    hash: &str,
) -> Result<bool, PasswordModuleError> {
...
pub(crate) async fn load_credential(
    postgres: &OwnedPostgres,
    identifier: &str,
) -> Result<Option<(String, String)>, PasswordModuleError> {
    let row = sqlx::query(
...
) -> Result<i64, PasswordModuleError> {
...
fn db(operation: &'static str) -> impl FnOnce(sqlx::Error) -> PasswordModuleError {
    move |source| PasswordModuleError::Database { operation, source }
}
